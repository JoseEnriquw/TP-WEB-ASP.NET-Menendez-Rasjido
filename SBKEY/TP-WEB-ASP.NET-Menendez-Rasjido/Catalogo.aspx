<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Catalogo.aspx.cs" Inherits="TP_WEB_ASP.NET_Menendez_Rasjido.Catalogo" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    
    <div class="jumbotron">
        <h1>NUESTROS PRODUCTOS</h1>
        <p class="lead">Hechale un vistazo a nuestros productos!</p>
    </div>

    <div class="container">
    <div class="row clearfix">
    <div class="col-lg-3 col-md-4 col-sm-12">

        <div class="card">
            <img src="https://imagenes.20minutos.es/files/og_thumbnail/uploads/imagenes/2021/03/02/ps5.jpeg" class="card-img-top" alt="..." />
            <div class="card-body">
                <h3 class="card-text-title">Producto 1</h3>
                <h5 class="card-text">Descripcion bla bla bla. Compralo plss</h5>
                <h6 class="card-text">$99.99 ARS</h6>
                <a href="Detalles de Productos" class="cart-btn-primary"><img class="cart-img-secundary" src="https://img.icons8.com/pastel-glyph/2x/info.png" alt="..." /> </a>
                <a href="Carrito de Compra" class="cart-btn-primary"><img class="cart-img-primary" src="https://img.icons8.com/material-sharp/452/add-shopping-cart.png" alt="..." /> </a> 
            </div>
        </div>
    </div>

     <div class="col-lg-3 col-md-4 col-sm-12">
        <div class="card">
            <img src="https://media.tenor.com/images/bcb63018f904222a7f17e0532a517269/tenor.gif" class="card-img-top" alt="..." />
            <div class="card-body">
                <h3 class="card-text-title">Producto 1</h3>
                <h5 class="card-text">Descripcion bla bla bla. Compralo plss</h5>
                <h6 class="card-text">$99.99 ARS</h6>
                <a href="Detalles de Productos" class="cart-btn-primary"><img class="cart-img-secundary" src="https://img.icons8.com/pastel-glyph/2x/info.png" alt="..." /> </a>
                <a href="Carrito de Compra" class="cart-btn-primary"><img class="cart-img-primary" src="https://img.icons8.com/material-sharp/452/add-shopping-cart.png" alt="..." /> </a> 
            </div>
        </div>
    </div>

    <div class="col-lg-3 col-md-4 col-sm-12">
        <div class="card">
            <img src="https://media.tenor.com/images/bcb63018f904222a7f17e0532a517269/tenor.gif" class="card-img-top" alt="..." />
            <div class="card-body">
                <h3 class="card-text-title">Producto 1</h3>
                <h5 class="card-text">Descripcion bla bla bla. Compralo plss</h5>
                <h6 class="card-text">$99.99 ARS</h6>
                <a href="Detalles de Productos" class="cart-btn-primary"><img class="cart-img-secundary" src="https://img.icons8.com/pastel-glyph/2x/info.png" alt="..." /> </a>
                <a href="Carrito de Compra" class="cart-btn-primary"><img class="cart-img-primary" src="https://img.icons8.com/material-sharp/452/add-shopping-cart.png" alt="..." /> </a> 
            </div>
        </div>
    </div>

    <div class="col-lg-3 col-md-4 col-sm-12">
        <div class="card">
            <img src="https://media.tenor.com/images/bcb63018f904222a7f17e0532a517269/tenor.gif" class="card-img-top" alt="..." />
            <div class="card-body">
                <h3 class="card-text-title">Producto 1</h3>
                <h5 class="card-text">Descripcion bla bla bla. Compralo plss</h5>
                <h6 class="card-text">$99.99 ARS</h6>
                <a href="Detalles de Productos" class="cart-btn-primary"><img class="cart-img-secundary" src="https://img.icons8.com/pastel-glyph/2x/info.png" alt="..." /> </a>
                <a href="Carrito de Compra" class="cart-btn-primary"><img class="cart-img-primary" src="https://img.icons8.com/material-sharp/452/add-shopping-cart.png" alt="..." /> </a> 
            </div>
        </div>
    </div>

    <div class="col-lg-3 col-md-4 col-sm-12">
        <div class="card">
            <img src="https://media.tenor.com/images/bcb63018f904222a7f17e0532a517269/tenor.gif" class="card-img-top" alt="..." />
            <div class="card-body">
                <h3 class="card-text-title">Producto 1</h3>
                <h5 class="card-text">Descripcion bla bla bla. Compralo plss</h5>
                <h6 class="card-text">$99.99 ARS</h6>
                <a href="Detalles de Productos" class="cart-btn-primary"><img class="cart-img-secundary" src="https://img.icons8.com/pastel-glyph/2x/info.png" alt="..." /> </a>
                <a href="Carrito de Compra" class="cart-btn-primary"><img class="cart-img-primary" src="https://img.icons8.com/material-sharp/452/add-shopping-cart.png" alt="..." /> </a> 
            </div>
        </div>
    </div>

    <div class="col-lg-3 col-md-4 col-sm-12">
        <div class="card">
            <img src="https://media.tenor.com/images/bcb63018f904222a7f17e0532a517269/tenor.gif" class="card-img-top" alt="..." />
            <div class="card-body">
                <h3 class="card-text-title">Producto 1</h3>
                <h5 class="card-text">Descripcion bla bla bla. Compralo plss</h5>
                <h6 class="card-text">$99.99 ARS</h6>
                <a href="Detalles de Productos" class="cart-btn-primary"><img class="cart-img-secundary" src="https://img.icons8.com/pastel-glyph/2x/info.png" alt="..." /> </a>
                <a href="#" class="cart-btn-primary"><img class="cart-img-primary" src="https://img.icons8.com/material-sharp/452/add-shopping-cart.png" alt="..." /> </a> 
            </div>
        </div>
    </div>

    <div class="col-lg-3 col-md-4 col-sm-12">
        <div class="card">
            <img src="https://media.tenor.com/images/bcb63018f904222a7f17e0532a517269/tenor.gif" class="card-img-top" alt="..." />
            <div class="card-body">
                <h3 class="card-text-title">Producto 1</h3>
                <h5 class="card-text">Descripcion bla bla bla. Compralo plss</h5>
                <h6 class="card-text">$99.99 ARS</h6>
                <a href="Detalles de Productos" class="cart-btn-primary"><img class="cart-img-secundary" src="https://img.icons8.com/pastel-glyph/2x/info.png" alt="..." /> </a>
                <a href="Carrito de Compra" class="cart-btn-primary"><img class="cart-img-primary" src="https://img.icons8.com/material-sharp/452/add-shopping-cart.png" alt="..." /> </a> 
            </div>
        </div>
    </div>

    <div class="col-lg-3 col-md-4 col-sm-12">
        <div class="card">
            <img src="https://media.tenor.com/images/bcb63018f904222a7f17e0532a517269/tenor.gif" class="card-img-top" alt="..." />
            <div class="card-body">
                <h3 class="card-text-title">Producto 1</h3>
                <h5 class="card-text">Descripcion bla bla bla. Compralo plss</h5>
                <h6 class="card-text">$99.99 ARS</h6>
                <a href="Detalles de Productos" class="cart-btn-primary"><img class="cart-img-secundary" src="https://img.icons8.com/pastel-glyph/2x/info.png" alt="..." /> </a>
                <a href="Carrito de Compra" class="cart-btn-primary"><img class="cart-img-primary" src="https://img.icons8.com/material-sharp/452/add-shopping-cart.png" alt="..." /> </a> 
            </div>
        </div>
    </div>
</div>
</div>

</asp:Content>
